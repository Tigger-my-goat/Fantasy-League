        ...
        if active_manager == "-- Join as New Manager --":
            ...
            with tabs[0]:
                ...
                with st.form("squad_form"):
                    ...
                    d4 = st.selectbox("Defender 4 (RB)", def_names, index=min(3, len(def_names)-1))

                    st.markdown("### ⚙️ Midfielders (4)")
                    mid_names = [p["name"] for p in pool["Midfielders"]]
                    m1 = st.selectbox("Midfielder 1", mid_names, index=0)
                    m2 = st.selectbox("Midfielder 2", mid_names, index=min(1, len(mid_names)-1))
                    m3 = st.selectbox("Midfielder 3", mid_names, index=min(2, len(mid_names)-1))
                    m4 = st.selectbox("Midfielder 4", mid_names, index=min(3, len(mid_names)-1))

                    st.markdown("### 🎯 Forwards (2)")
                    fwd_names = [p["name"] for p in pool["Forwards"]]
                    f1 = st.selectbox("Forward 1", fwd_names, index=0)
                    f2 = st.selectbox("Forward 2", fwd_names, index=min(1, len(fwd_names)-1))

                    submit_squad = st.form_submit_button("Save Starting 11 & Submit")

                if submit_squad:
                    selected_list = [gk_selected, d1, d2, d3, d4, m1, m2, m3, m4, f1, f2]
                    
                    # Calculate Points
                    total_score = 0
                    all_players = pool["Goalkeepers"] + pool["Defenders"] + pool["Midfielders"] + pool["Forwards"]
                    for p_name in selected_list:
                        for p in all_players:
                            if p["name"] == p_name:
                                total_score += p["pts"]

                    mgr_data["squad"] = selected_list
                    mgr_data["points"] = total_score
                    save_db(db)
                    st.success("Starting 11 saved successfully!")
                    st.rerun()

                # Display Saved 11 Pitch Cards
                if squad:
                    st.divider()
                    st.markdown("### 🏟️ Your Active Lineup")
                    cols = st.columns(4)
                    all_players = pool["Goalkeepers"] + pool["Defenders"] + pool["Midfielders"] + pool["Forwards"]
                    for idx, p_name in enumerate(squad):
                        player_info = next((p for p in all_players if p["name"] == p_name), None)
                        if player_info:
                            with cols[idx % 4]:
                                st.markdown(f"""
                                <div class="player-card">
                                    <img src="{player_info['img']}" class="player-avatar"/>
                                    <h4>{player_info['name']}</h4>
                                    <p style="color:#38bdf8;">{player_info['club']}</p>
                                    <div>
                                        <span class="stat-tag">OVR: {player_info['rating']}</span>
                                        <span class="stat-tag">PTS: {player_info['pts']}</span>
                                    </div>
                                </div>
                                """, unsafe_allow_html=True)
            ...
            with tabs[1]:
                ...
                sorted_members = sorted(league["members"].items(), key=lambda x: x[1]["points"], reverse=True)
                for rank, (name, data) in enumerate(sorted_members, 1):
                    squad_count = len(data.get("squad", []))
                    st.markdown(f"### #{rank} **{name}** — `{data['points']} pts` ({squad_count}/11 players selected)")

            # TAB 3: STATS EXPLORER
            with tabs[2]:
                st.subheader(f"📊 {league['tournament']} Player Database & Stats")
                for pos, players in pool.items():
                    ...
                    for i, p in enumerate(players):
                        with p_cols[i % 3]:
                            st.markdown(f"""
                            <div class="player-card">
                                <img src="{p['img']}" class="player-avatar"/>
                                <h4>{p['name']}</h4>
                                <p style="color:#a855f7;">{p['club']}</p>
                                <div>
                                    <span class="stat-tag">⚽ Goals: {p['goals']}</span>
                                    <span class="stat-tag">🎯 Assist: {p['assists']}</span>
                                    <span class="stat-tag">🛡️ CS: {p['clean_sheets']}</span>
                                </div>
                            </div>
                            """, unsafe_allow_html=True)
    else:
